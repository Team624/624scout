...
CREATE TEMPORARY TABLE IF NOT EXISTS agg_result ENGINE = MEMORY ( 
  ...
  ROUND(SUM(tele_high_score) / sum(case when no_show = 0 then 1 end), 2) as 'tele_high_score',
  ROUND(SUM(tele_high_miss) / sum(case when no_show = 0 then 1 end), 2) as 'tele_miss_high',
  ROUND(SUM(tele_high_miss+tele_high_score) / sum(case when no_show = 0 then 1 end), 2) as 'shots_high',
  ROUND(SUM(tele_low_score) / sum(case when no_show = 0 then 1 end), 2) as 'tele_low_score',
  ROUND(SUM(tele_low_miss) / sum(case when no_show = 0 then 1 end), 2) as 'tele_low_miss',
  ROUND(SUM(tele_low_miss+tele_low_score) / sum(case when no_show = 0 then 1 end), 2) as 'shots_low',
  ROUND(SUM(truss) / sum(case when no_show = 0 then 1 end), 2) as 'truss',
  ROUND(SUM(truss_miss) / sum(case when no_show = 0 then 1 end), 2) as 'truss_miss',
  ROUND(SUM(catch) / sum(case when no_show = 0 then 1 end), 2) as 'catch',
  ROUND(SUM(catch_miss) / sum(case when no_show = 0 then 1 end), 2) as 'catch_miss'
FROM matches_result
